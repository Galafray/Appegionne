...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure edEventsUsedKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
begin
  ...
  begin
    if MidiNoteVals[Index] = Note then
    begin
      GetMidiNote := Index;
      break;
    end;
...
var Chn: integer;
  ...
  begin
    ...
    begin
      if LeftStr(InstArraySC88[InstInx], 8) = aStr then
      begin
...
begin
  ...
  x := 1580; {Over the Sibelius scroll-bar right bottom}
  y := 1050;
  MouseInput.Click(mbLeft, [], x, y);
  Application.ProcessMessages;
...
procedure TForm1.edEventsUsedKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
var TheRow: integer;
begin
  if Chr(Key) = #13 Then begin
    if edEventsUsed.Text <> '' then begin
      TheRow := StrToInt(edEventsUsed.Text);
      if TheRow <= sgEventTable.RowCount then begin
        labEventsUsed.Caption := IntToStr(TheRow);
...
//{Assumes serial started. Send the first 400 records, or up to EventCount}
...
begin
  ...
  //EventIx := 400;  {Hex 0190 is the usual amount to request}
  EventIx := 600;  {Hex $258 is the usual amount to request}
  //for EventIx := 1 to EventIx do {Only from the start of the array}
  //for EventIx := StartEvent to StartEvent + 399 do
  btnSend.Enabled := false;
  Application.ProcessMessages;
  for EventIx := StartEvent to StartEvent + 599 do begin
    if EventIx <= EventCount then {Don't overshoot the array.}
    begin
      with EventRecArray[EventIx] do
      begin
...
begin
  ...
  StartEvent := StrToInt(edStartEvent.Text);
  if StartEvent > EventCount - 1 then begin
  ...
  if EventRecArray[StartEvent].WhichSynth > $00 then begin
...
begin
  ...
  if aStr = 'PageF' then begin
    x := 1580; {Over the Sibelius scroll-bar bottom right}
    y := 1050;
  end else begin {PageB}
    x := 680; {Over the Sibelius scroll-bar bottom left}
    y := 1050;
  end;
  MouseInput.Click(mbLeft, [], x, y);
      ...
      case ch of
        ...
        begin //Cariage return
          ...
          if (leftStr(ComIn, 2) = '21')
            and (length(ComIn) = 8) then begin {Arduino says 'Display this packet'}
            DisplayPacket(ComIn);
          end;
...
begin
  //midiFileName := 'C:\Arpeggione\ArdFiles\Recorder duet.mid';
  //midiFileName := 'C:\Arpeggione\MidFiles\Recorder duet.mid';
  midiFileName := 'C:\Arpeggione\Scores\MidFiles\Cox George Clarinet nocturn.mid';
  LoadMidiFile;
end;
...
begin
  ...
  DisplayEventGrid();
  labStatus.caption := 'File read in.';
  Form1.Caption := 'Arpeggionne ' + ExtractFileName(ardFileName);
...
It assumes serial has started  ===   not used}
var
  s: String;
begin
  s := '051zzzzz'; {050zzzzz turns it off}
  Transmit(s);

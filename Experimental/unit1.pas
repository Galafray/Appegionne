...
type
  ...
  EventType = Record
    ...
    Synth: integer; {Which sound module}
    Byte1: byte; {Channel, Command, Auto-play}
    Byte2: byte; {Note}
    Byte3: byte; {Velocity}
  ...
  TForm1 = class(TForm)
    ...
    procedure btnReportStatusClick(Sender: TObject);
    ...
    procedure TransmitPacket(E: EventType);
  ...
  for EventIx := 1 to EventCount do begin
    with Events[EventIx] do begin
      ...
      if Byte1 = $FF then begin
        Chn := Byte2 - $C0;
        if not InRange(Events[EventIx - 2].Byte1, $B0, $BF) then break;
        if not InRange(Events[EventIx - 1].Byte1, $B0, $BF) then break;
        case Chn of
          $0..$F: begin
            Inst := Byte3; {The order these appear unfortunatly varies}
            LSB := Events[EventIx - 1].Byte3;
            BankVar := Events[EventIx - 2].Byte3;
          end;
        end;
      ...
      if (Byte2 = $0A) then begin {Panpot. $40 is centre}
        Chn := Byte1 - $B0;
        case Chn of
          $0..$F: begin
            PanPot := Byte3;
  ...
  for PrevRow := CurrRow - 1 downTo 1 do begin
    if Events[PrevRow].Synth = 0 then begin
      ...
      with Events[PrevRow] do begin
        Events[PrevRow].Byte1 := (NewDelta and $FF0000) div $FFFF;
        {This is hardly needed and it would overwrite auto}
        Events[PrevRow].Byte2 := (NewDelta and $00FF00) div $FF;
        Events[PrevRow].Byte3 := (NewDelta and $0000FF);
        {Display the change}
        with sgEventTable do begin
          Cells[2, PrevRow] := IntToStr(NewDelta);
          Cells[4, PrevRow] := intToHex(Byte1, 2);
          Cells[5, PrevRow] := intToHex(Byte2, 2);
          Cells[6, PrevRow] := intToHex(Byte3, 2);
        end;
      end;
      exit;
    end;
...
begin
  ...
  for EventIx := 1 to EventCount - 1 do begin
    with Events[EventIx] do begin
      if Byte1 = $90 then begin {cmd}
        inc(Notes[Byte2]); {note}
        if Byte2 = $42 then begin
          Byte2 := $42;
        end;
      end;
      if Byte1 = $80 then begin
        dec(Notes[Byte2]);
      end;
    end;
  end;
  ...
  for Ix := 0 to 120 do begin
    if Notes[Ix] <> $00 then begin
      labStatus.Caption := 'Problem with ' +
        HexStr(Ix, 2) + ' ' + MidiNoteVals[Ix] + '.';
      //exit;
    end;
  end;
...
begin
  ...
  Events[CurrRow].Byte1 := $00;
  Events[CurrRow].Byte2 := $00;
  Events[CurrRow].Byte3 := $00;
  ...
  if (edAutoFrom.Caption = '') OR (edAutoTo.Caption = '') then begin
    ...
    for EventIx := 2 to EventCount do begin {Skip top one}
      if Events[EventIx].Synth > $00 then begin
        {A non-timing record}
        if (Events[EventIx].byte1 >= $90) and
          (Events[EventIx].byte1 <= $9F) then
            NoteOnFound := true;
      end else begin
        {A timing record}
        if (NoteOnFound = false) and (HoldIndex > 1) then begin
          Events[HoldIndex].byte1 := $01; {Cmd. Record made automatic}
        end;
        HoldIndex := EventIx;
        NoteOnFound := false;
      end;
...
begin
  ...
  Events[CurrRow].Byte1 := $02;
  Events[CurrRow].Byte2 := $FF;
  Events[CurrRow].Byte3 := $FF;
  ...
  for EventIx := CurrRow downto 1 do begin
    if Events[EventIx].Synth = 0 then begin
      ...
      if Events[EventIx].Byte1 = 0 then begin
...
begin
  ...
  Events[CurrRow].Synth := StrToInt(EdC1.Text); {WhichSynth}
  Events[CurrRow].Byte1 := Hex2Dec(EdC2.Text); {Channel}
  Events[CurrRow].Byte2 := Hex2Dec(EdC3.Text); {Note}
  Events[CurrRow].Byte3 := Hex2Dec(EdC4.Text); {Velocity}
  DisplayEventGrid;
  sgEventTable.Row := CurrRow;
end;
...
begin
  ...
  Events[CurrRow].Synth := 4;
  Events[CurrRow].Byte1 := $02;
  Events[CurrRow].Byte2 := $AA;
  Events[CurrRow].Byte3 := $AA;
...
{Quieten the synths}
  ...
  for F := EventsUsed - 1 downto 1 do begin
    if Events[F].Synth = $04 then begin
      if Events[F].Byte1 = $02 then begin
        if Events[F].Byte2 = $AA then begin {A practice point}
        ...
        if Events[F].Byte2 = $FF then begin {A page forward found}
  ...
  for F := EventsUsed + 1 to EventCount do begin
    if Events[F].Synth = $04 then begin
      if Events[F].Byte1 = $02 then begin
        ...
        if Events[F].Byte2 = $FF then begin {A page forward}
          HoldX := aMouse.CursorPos.X;
          HoldY := aMouse.CursorPos.Y;
        ...
        if Events[F].Byte2 = $BB then begin {A page back}
          HoldX := aMouse.CursorPos.X;
          HoldY := aMouse.CursorPos.Y;
...
procedure TForm1.btnReportStatusClick(Sender: TObject);
var
  Packet: EventType;
begin
  Packet.Synth := 9;
  ...
  Packet.Byte2 := 0;
  Packet.Byte3 := 0;
  labStatus.Caption := 'Status requested';
...
begin
  ...
  begin
    if Events[EventIx].Byte1 = $FF then
    begin
      {This is an instrument change. What channel?}
      Chn := Events[EventIx].Byte2 and $0F;
      if  cgChannel.Checked[Chn] then begin
        Events[EventIx - 0].Byte3 :=
          Hex2Dec(sgIntegra7.Cells[4, sgIntegra7.Row]);
        Events[EventIx - 1].Byte3 :=
          Hex2Dec(sgIntegra7.Cells[3, sgIntegra7.Row]);
        Events[EventIx - 2].Byte3 :=
...
begin
  ...
  if cbAllowPointer.Checked then begin
    Packet.Byte3 := 1;
    labStatus.Caption := 'Pointer allowed';
  end else begin
    Packet.Byte3 := 0;
    labStatus.Caption := 'Pointer disallowed';
  end;
  TransmitPacket(Packet);
end;
    ...
    while midArrIx < trackRecArray[trackIndex].trackEnd do begin
      ...
      case midiArr[midArrIx] of
        $B0..$BF: begin {Control/Mode change. Just send these 3 bytes}
          ...
          Events[EventCount].Byte1 := midiArr[midArrIx];
          inc(midArrIx);
          Events[EventCount].Byte2 := midiArr[midArrIx];
          inc(midArrIx);
          Events[EventCount].Byte3 := midiArr[midArrIx];
          inc(midArrIx);
        end;
        $C0..$CF: begin {Instrument change}
          Events[EventCount].Synth := 2; {General midi};
          Events[EventCount].Byte1 := $FF;
          Events[EventCount].Byte2 := midiArr[midArrIx];
          inc(midArrIx);
          Events[EventCount].Byte3 := midiArr[midArrIx];
          inc(midArrIx);
...
begin
  CurrRow := sgEventTable.Row;
  if Events[CurrRow].DeltaTime = $00 then
    exit; {Can only do this to timing records}
  if Events[CurrRow].Byte1 = $00 then
    Events[CurrRow].Byte1 := $01
  else
    Events[CurrRow].Byte1 := $00;
  DisplayEventGrid;
  sgEventTable.Row := CurrRow;
...
begin
  with Packet do begin
    ...
    Byte1 := 0;
    Byte2 := 0;
    Byte3 := rgKeyboard.ItemIndex;
    (*
      0: Byte3 := 0; {Trigger}
      1: Byte3 := 0; {Through to Integra}
      2: Byte3 := 0; {Through to SC-88}
    *)
  end;
  TransmitPacket(Packet);
...
//  end;
  //Transmit(s);
  labStatus.Caption := 'Through or trigger';
end;
...
procedure TForm1.TransmitPacket(E: EventType);
var
  status: LongInt;
  writecount: integer;
begin
  //labStatus.Caption := 'Sending a packet';
  //Application.ProcessMessages;
  //writecount := length(s);
  writecount := 1;
  status := SerWrite(SerialHandle, E.Synth, writecount);
  status := SerWrite(SerialHandle, E.Byte1, writecount);
  status := SerWrite(SerialHandle, E.Byte2, writecount);
  status := SerWrite(SerialHandle, E.Byte3, writecount);
end;
...
var
  aLongInt: longint;
  Bytes: array[0 .. 3] of byte absolute aLongInt; {MSB is at [0]}
  Packet: EventType;
begin
  if EventCount = 0 then begin
  ...
  with txMidiOut do begin
    ...
    //Transmit(s);
    Packet.Synth := 7;
    Packet.Byte1 := Bytes[2];
    Packet.Byte2 := Bytes[1];
    Packet.Byte3 := Bytes[0]; {Hopefully this is big enough to hold it}
    TransmitPacket(Packet);
...
begin
  ...
  if Events[txMidiOut.StartEventWanted].Byte1 > $00 then begin
...
begin
  ...
  if Events[Used].Byte3 = 170 then begin {'AA' practice-point}
...
var
  ...
  begin
    ...
    if txMidiOut.StartEventWanted = 0 then begin {200zzzz as opposed to 201zzzz}
      ...
      //Transmit(s);
      aLongInt := txMidiOut.StartEventWanted;
      Packet.Synth := 7;
      Packet.Byte1 := Bytes[2];
      Packet.Byte2 := Bytes[1];
      Packet.Byte3 := Bytes[0]; {Hopefully this is big enough to hold it}
...
begin
  ...
  s := s + intToHex(Events[EventIx].Byte1, 2);
  s := s + intToHex(Events[EventIx].Byte2, 2);
  s := s + intToHex(Events[EventIx].Byte3, 2);
  labStatus.Caption := s;
  Transmit(s);
end;
*)
...
procedure TForm1.btnLoadClick(Sender: TObject); {On the Arduino tab}
begin
  //AllSoundOff;
  //LoadNoteList;
  //if glSerialRunning then
...
begin
  ...
  For EventIx := 1 to EventCount -1 do begin
    if (Events[EventIx].Byte1 = $02)
      and (Events[EventIx].Byte2 = $AA) then begin
      {Practice point}
...
procedure TForm1.btnLoadNoteListClick(Sender: TObject);  {On the Note list tab}
begin
  //AllSoundOff;
  //LoadNoteList;
...
begin
  if not glSerialRunning then exit;
  for Chan := 0 to 15 do begin {Turn off each channel in turn in both synths}
    with Packet do begin
      ...
      TransmitPacket(Packet);
      {Synth 2 SC-88}
      Synth := 12; {The other ard the same}
      TransmitPacket(Packet);
    end;
  end;
  labStatus.Caption := 'All sound off.';
  {And now an addition way to mute}
  with Packet do begin
    Synth := 5;
    Byte1 := $00;
    Byte2 := $00;
